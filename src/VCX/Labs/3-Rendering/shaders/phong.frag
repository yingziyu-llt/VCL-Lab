...
vec3 Shade(vec3 lightIntensity, vec3 lightDir, vec3 normal, vec3 viewDir, vec3 diffuseColor, vec3 specularColor, float shininess) {
    ...
    if (!u_UseBlinn) {
        ...
        vec3 r = 2 * normal * dot(normal, lightDir) - lightDir;
        vec3 Ls = specularColor * lightIntensity * pow(max(0, dot(r, viewDir)), shininess);
        res = Ld + Ls;
    } else {
        vec3 Ld = diffuseColor * lightIntensity * max(0, dot(lightDir, normal));
        vec3 h = normalize(lightDir + viewDir);
        vec3 Ls = specularColor * lightIntensity * pow(max(0, dot(normal, h)), shininess);
        res = Ld + Ls;
    }
    return res;
}
